...
export function handleCommand(cmd, config, setters) {
    ...
    if (numericCommands[keyword]) {
        if (isNaN(val) || val <= 0) {
            return { done: true, msg: `Nilai '${keyword}' harus berupa angka > 0.` };
        }
        ...
        if (typeof setters[setterName] === "function") {
            ...
            setters.saveConfig();
            return { done: true, msg: `${numericCommands[keyword].label} diset ke ${val}.` };
        } else {
            return { done: true, msg: `Setter ${setterName} tidak ditemukan.` };
        }
    }
